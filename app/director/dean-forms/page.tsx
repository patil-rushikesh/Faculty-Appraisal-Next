...
export default function DirectorDeanFormsPage() {
    ...
    const fetchAppraisals = async () => {
      ...
      try {
        const response = await axios.get(
          `${BACKEND}/appraisal/by-role/dean`,
          {
            headers: {
        ...
        if (response.data.success) {
          ...
          setError(response.data.message || "Failed to fetch Dean appraisals");
        }
      } catch (err: any) {
        console.error("Error fetching Dean appraisals:", err);
        setError(err.response?.data?.message || "Failed to fetch Dean appraisals");
      } finally {
        setLoading(false);
      }
    };
  ...
  const summaryCards = [
    { label: "Total Deans", value: summary.total ?? 0, cls: "bg-card border" },
  ...
  if (loading) {
    return <Loader message="Loading Dean appraisals..." />;
  }
            ...
            <TableRow className="bg-muted/50">
              <TableHead className="cursor-pointer select-none" onClick={() => toggleSort("name")}>
                <span className="inline-flex items-center gap-1">
                  Dean <SortIcon col="name" />
                </span>
              </TableHead>
              <TableHead>Department</TableHead>
              <TableHead>Designation</TableHead>
              <TableHead>Status</TableHead>
              <TableHead className="cursor-pointer select-none text-right" onClick={() => toggleSort("marks")}>
