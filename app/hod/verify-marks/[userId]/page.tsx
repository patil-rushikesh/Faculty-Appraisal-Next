    ...
    const fetchData = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          // Map appraisal data to expected format
        ...
        toast({
          title: "Error",
          description: error.response?.data?.message || "Failed to load faculty data",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
    ...
    return (
      <div className="container mx-auto p-6">
        <div className="text-center text-red-600">
          <p className="text-lg">Failed to load faculty data</p>
        </div>
      </div>
    );
  }
